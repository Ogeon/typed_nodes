...
impl LuaType {
    ...
    pub(crate) fn make_delegating_visitor_fn(
        ...
        match self {
            ...
            LuaType::Number => {
                quote! {
                    fn visit_number(&mut self, value: f64, context: &mut typed_nodes::mlua::Context<'lua, #bounds_type>) -> mlua::Result<Self::Output> {
                        #body
                    }
                }
            }
            LuaType::Integer => {
                quote! {
                    fn visit_integer(&mut self, value: i64, context: &mut typed_nodes::mlua::Context<'lua, #bounds_type>) -> mlua::Result<Self::Output> {
                        #body
                    }
                }
            }
